...
if [[ "$old_volume_id" == "$new_volume_id" ]]; then
  echo "something went wrong - old_volume_id ${old_volume_id} is the same as new"
...
ssh linux.norbert-logiewa.co.uk "sudo mkdir -p $efs_mount_point && sudo apt update && sudo apt install -y git nfs-{common,kernel-server} curl wget vim sed && sudo mount -t nfs -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport ${efs_id}.efs.${region}.amazonaws.com:/ $efs_mount_point"
