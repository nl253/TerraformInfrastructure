...
terraform apply -auto-approve "${plan_file}" 1>/dev/null || { terraform destroy -auto-approve -var "app_name=${app_name}" -var "cidr_vpc=${cidr_vpc}" -var "cidr_public=${cidr_public}" -var "cidr_private=${cidr_private}" 1>/dev/null; exit 1; }
